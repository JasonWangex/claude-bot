...
- **子任务拆解**：按**功能点**拆分，不按技术层拆分
  ...
  - 可选使用 Phase 分组
- **子任务详细计划**：每个子任务必须展开为独立的 section，包含：
  - **目标**：一句话说明这个任务要做什么
  - **动机（为什么）**：解释为什么要这样做，帮助执行者理解设计意图
  - **具体实现**：新增/修改的文件清单、关键数据结构、核心逻辑要点
  - **注意事项**（可选）：边界情况、兼容性、已知风险
- **关键决策**：记录讨论中做出的重要决策
...
### t2: 功能点描述 `[代码, complex]` — depends: t1
