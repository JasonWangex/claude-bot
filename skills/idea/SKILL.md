...
name: idea
description: >
  快速记录想法或推进已有想法到开发。有参数时直接记录（Status=Idea）；
...
将一句话想法直接写入数据库，Status 设为 Idea。

**不讨论、不确认、不追问**，直接调用 API 写入：
...
```bash
curl -s -H "$AUTH" "$API/api/goals?status=Idea&project=<根据 cwd 判断的项目名>"
```
