...
export function registerGoalTaskTools(server: McpServer) {
  server.registerTool('bot_goal_tasks', {
    title: 'Goal Tasks',
    description: 'Goal sub-task management. action: list (tasks in a drive), set (bulk-write initial tasks before drive), skip, done, retry, refix, pause, resume, nudge (light-push to let agent self-assess and continue).',
    inputSchema: {
      action: z.enum(['list', 'set', 'skip', 'done', 'retry', 'refix', 'pause', 'resume', 'nudge']).describe('Operation type'),
