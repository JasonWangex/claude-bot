...
export class GoalOrchestrator {
  ...
  async startDrive(params: StartDriveParams): Promise<GoalDriveState> {
    ...
    if (existing && (existing.status === 'running' || existing.status === 'paused')) {
      ...
      await this.notify(goalThreadId, `Goal "${goalName}" is already ${existing.status}.${hint}`);
      return existing;
    }
  ...
  private async doMergeAndCleanup(state: GoalDriveState, task: GoalTask): Promise<void> {
    ...
    try {
      ...
      const goalWorktreeDir = this.findWorktreeDir(stdout, state.goalBranch);
      if (!goalWorktreeDir) {
        await this.notify(state.goalThreadId, `Cannot find goal worktree, skipping merge: ${branchName}`);
        return;
      }
      ...
      if (result.success) {
        task.merged = true;
        saveState(state);
        await this.notify(state.goalThreadId, `Merged: \`${branchName}\` → \`${state.goalBranch}\``);
